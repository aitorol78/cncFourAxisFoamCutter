...
# cuts two stacked profiles out of the same foam panel
...
if __name__ == "__main__":
    ...
    # create trayectory for an inverted and stacked wing
    SecondWingTemplateHeigth = minDistanceBetweenWings + 2*np.max([tr.sectionPlusTrayectoryY, tr.sectionMinusTrayectoryY])

    sectionLayoutZplus2 = copy.deepcopy(sectionLayoutZplus)
    sectionLayoutZplus2.flagInvertProfile = True
    sectionLayoutZplus2.templateHeight = SecondWingTemplateHeigth

    sectionLayoutZminus2 = copy.deepcopy(sectionLayoutZminus)
    sectionLayoutZminus2.flagInvertProfile = True
    sectionLayoutZminus2.templateHeight = SecondWingTemplateHeigth

    wpl2 = copy.deepcopy(wpl)
    wpl2.sectionPlusLayout = sectionLayoutZplus2
    wpl2.sectionMinusLayout = sectionLayoutZminus2
 
    tr2 = trayectoryGenerator()

    tr2.machine = mch
    tr2.wpl = wpl2
    tr2.numStationsUpperSurface = tr.numStationsLowerSurface
    tr2.numStationsLowerSurface = tr.numStationsUpperSurface
    tr2.velocity = tr.velocity
    tr2.generateTrayectory()

    # add tr2 to tr
    tr.wirePlusTrayectoryX = np.hstack([0, tr2.wirePlusTrayectoryX, tr.wirePlusTrayectoryX])
    tr.wirePlusTrayectoryY = np.hstack([0, tr2.wirePlusTrayectoryY, tr.wirePlusTrayectoryY])
    tr.wireMinusTrayectoryX = np.hstack([0, tr2.wireMinusTrayectoryX, tr.wireMinusTrayectoryX])
    tr.wireMinusTrayectoryY = np.hstack([0, tr2.wireMinusTrayectoryY, tr.wireMinusTrayectoryY])

    # remove abosulute Xedge offset
    tr.wirePlusTrayectoryX[1:] = tr.wirePlusTrayectoryX[1:] - tr.wirePlusTrayectoryX[1]
    #tr.wirePlusTrayectoryY = tr.wirePlusTrayectoryY - tr.wirePlusTrayectoryY[0]
    tr.wireMinusTrayectoryX[1:] = tr.wireMinusTrayectoryX[1:] - tr.wireMinusTrayectoryX[1]
    ...
    print('\n')
    print(('plus  min Y (mm): {:.1f}').format(min(tr.wirePlusTrayectoryY)))
    print(('minus  min Y (mm): {:.1f}').format(min(tr.wireMinusTrayectoryY)))
    print('\n')
    print(('plus  max Y (mm): {:.1f}').format(max(tr.wirePlusTrayectoryY)))
    print(('minus  max Y (mm): {:.1f}').format(max(tr.wireMinusTrayectoryY)))
    print('\n')
    ...
    axs[0,0].plot(tr2.sectionMinusTrayectoryX, tr2.sectionMinusTrayectoryY,'black')
    axs[1,0].plot(tr.wireMinusTrayectoryX, tr.wireMinusTrayectoryY,'red')
    axs[0,1].plot(tr.sectionPlusTrayectoryX, tr.sectionPlusTrayectoryY,'black')
    axs[0,1].plot(tr2.sectionPlusTrayectoryX, tr2.sectionPlusTrayectoryY)
